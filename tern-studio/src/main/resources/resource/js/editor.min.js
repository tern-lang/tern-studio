...
M.StatusPanel.showActiveFile(g.getEditorResource().getProjectPath());a.showEditorFileInTree();p();r(g.getEditorResource().getResourcePath())};a.focusEditor=function(){g.getEditorPanel().focus();g.getEditorPanel().setReadOnly(!1)};a.setReadOnly=l;a.showEditorFileInTree=function(){var a=b().getResource();n.FileTree.showTreeNode("explorerTree",a)};a.getCurrentLineForEditor=function(){return g.getEditorPanel().getSelectionRange().start.row};a.getSelectedText=function(){return g.getEditorPanel().getSelectedText()};
a.isEditorChanged=f;a.isEditorChangedForPath=k;a.scrollEditorToPosition=p;a.updateProjectTabOnChange=function(){g.getEditorPanel().on("input",function(){var a=g.getEditorResource(),a=g.getHistoryForResource(a),b=g.getEditorPanel().getValue();a.touchHistory(b);r(g.getEditorResource().getResourcePath())})};a.formatEditorSource=function(){q.Command.formatEditorSource()};a.setEditorTheme=function(a){null!=a&&null!=g.getEditorPanel()&&g.getEditorPanel().setTheme(a)};a.showEditorBreakpoints=function(){g.getEditorBreakpointManager().showEditorBreakpoints()};
a.updateEditorFont=function(a,b){var c;c=a.split(",");for(var d="",f=0;f<c.length;f++)var h=c[f],h=e.Common.stringReplaceText(h,"'",""),h=e.Common.stringReplaceText(h,'"',""),h="'"+h.trim()+"'",d=d+(h+",");c=d+"monospace";g.getEditorPanel().setOptions({enableBasicAutocompletion:!0,enableLiveAutocompletion:!0,fontFamily:c,fontSize:b})}})(v=h.FileEditor||(h.FileEditor={}))});
