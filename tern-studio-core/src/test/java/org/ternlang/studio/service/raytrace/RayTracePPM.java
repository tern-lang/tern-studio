package org.ternlang.studio.service.raytrace;
...
public class RayTracePPM {
  ...
  public static Vector3 trace(Ray ray, int depth) {
    ...
    if (did_hit && depth < MAX_DEPTH) {
      if (sp.is_light != true) {
        Ray nray = new Ray(
            hit.point,
            RayTracePPM.rnd_dome(hit.normal)
            );
        Vector3 ncolor = RayTracePPM.trace(nray, depth + 1);
  ...
  public static void WritePPM (Vector3[][] data) {
     try {
      PrintWriter ppm = new PrintWriter("c:/Work/temp/javarb.ppm", "UTF-8");  
      ppm.write(String.format("P3\n%d %d\n255\n", RayTracePPM.WIDTH, RayTracePPM.HEIGHT));
  ...
  public static void main (String[] args) {
    ...
    Vector3[][] data = new Vector3[RayTracePPM.HEIGHT][];
    Camera cam = new Camera();
    Vector3 vdu = cam.rt.Sub(cam.lt).Div(RayTracePPM.WIDTH);
    Vector3 vdv = cam.lb.Sub(cam.lt).Div(RayTracePPM.HEIGHT);

    for(int y = 0; y < RayTracePPM.HEIGHT; ++y) {
      data[y] = new Vector3[RayTracePPM.WIDTH];
      for(int x = 0; x < RayTracePPM.WIDTH; ++x) {
        Vector3 color = new Vector3();
        Ray ray = new Ray();

        ray.origin = cam.eye;

        for(int i = 0; i < RayTracePPM.SAMPLES; ++i) {
          ...
          color = color.Add(RayTracePPM.trace(ray, 0));
        }

        color = color.Div(RayTracePPM.SAMPLES);

        data[y][x] = color;
      }
    }

    RayTracePPM.WritePPM(data);
  }
}
